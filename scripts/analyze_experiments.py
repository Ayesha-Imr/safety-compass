#!/usr/bin/env python3
"""Analyze multi-experiment results and produce publication-ready figures.
...
Usage:
    ...
    python scripts/analyze_experiments.py \
        --csv "Alpaca=results/exp_alpaca/drift_log.csv" \
              "Dolly=results/exp_dolly/drift_log.csv" \
              "Code Alpaca=results/exp_code_alpaca/drift_log.csv" \
        --output-dir results/analysis/
"""
...
from safety_compass.utils import DRIFT_THRESHOLD, read_csv_rows, to_float
...
def compute_drift_onset(rows: list[dict], concept: str, threshold: float = DRIFT_THRESHOLD):
    ...
    for row in rows:
        if to_float(row[column]) < threshold:
            return int(to_float(row["step"]))
    return None
...
def compute_experiment_summary(rows: list[dict], concepts: list[str]) -> dict:
    ...
    for concept in concepts:
        ...
        cosines = [to_float(r[cosine_col]) for r in rows]
        aurocs = [to_float(r[auroc_col]) for r in rows]
        ...
        summary[concept] = {
            ...
            "step_at_min_cosine": int(to_float(rows[min_cos_idx]["step"])),
        ...
        if norm_col in rows[0]:
            norms = [to_float(r[norm_col]) for r in rows]
...
def print_drift_onset_table(all_summaries: dict[str, dict], concepts: list[str]):
    ...
    print(f"\nDrift Onset Table (step where cosine < {DRIFT_THRESHOLD})")
    print("=" * len(header))
    print(header)
    print("-" * len(header))
    for concept in concepts:
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze multi-experiment drift results",
    ...
    analysis_path = output_dir / "experiment_analysis.json"
    ...
    try:
        plot_experiment_comparison(experiment_csvs, str(output_dir), concepts=concepts)
